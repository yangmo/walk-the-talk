...
- **R4**：测试目录按 phase 分子目录（`tests/{ingest,extract,verify,report}/`），git mv 保 history；新建 `tests/conftest.py` 提供共享 fixture（`fixtures_dir` / `smic_html_path` / `smic_data_dir`），消除 5 处散落的 `Path(__file__).parent / "fixtures" / ...`；新建 `tests/verify/test_rescue.py` 单测 P4 rescue 边界（14 个 case）。README 重写（中文为主 + 括号内英文术语，"问题→架构→免责声明" 顺序）。
- **R5**：工程化基线
  - 新增 GitHub Actions CI（`.github/workflows/ci.yml`）：lint job + test 矩阵（Python 3.10/3.11/3.12）；pip cache + 覆盖率 artifact。
  - 新增 `.pre-commit-config.yaml`（ruff format/check + 通用文件卫生 hook + 大文件检测）。
